function colorPlot(goID, graphID){
    let graph = d3.select(graphID);
    console.dir(graph);
    graph.selectAll(".dot")
        .classed("highlight", function(d){
            let goArray = d['GO_terms'];
            if (!Array.isArray(goArray))
                return false;
            if (goArray.includes(goID))
                return true;
            return false            
        })
}
...
function volcanoPlot() {
    var width = 960,
        ...
        yAxisLabelOffset,
        foldChangeThresholdLeft, // = -1.8, // fold change level to colour by
        foldChangeThresholdRight, // = 0.8, // fold change level to colour by
        significanceThreshold, // = document.getElementById("myRange").value, // = 1.3, 
    ...
    // Create circle foreach line in tsv
            ...
            [foldChangeThresholdRight, foldChangeThresholdLeft].forEach(function(threshold) {
                thresholdLines.append("svg:line")
                    .attr('class', 'threshold')
            ...
            function tipEnter(d) {
                tooltip.style('visibility', 'visible')
                    ...
                    .html(
                        ...
                        '<strong>' + zColumn + '</strong>: ' + d[zColumn].slice(0, 3) +'..... '
                    );
            }
            ...
            function yTickFormat(n) {
                return d3.format(".2r")(n); //getBaseLog(10, n));
            ...
            function circleClass(d) { //  THE DOT COLORS AND THRESHOLD VALUES ARE LINKED HERE
                if ((d[yColumn]<=significanceThreshold && d[xColumn]>=foldChangeThresholdRight)
                || (d[yColumn]<=significanceThreshold && d[xColumn]<=foldChangeThresholdLeft)) 
                    return 'dot sigfold';
                else if (d[yColumn] <= significanceThreshold) return 'dot sig';
                else if ((d[xColumn]>=foldChangeThresholdRight)||(d[xColumn]<=foldChangeThresholdLeft)) return 'dot fold';
                else return 'dot';
            }
    ...
    chart.foldChangeThresholdRight = function(value) {
        if (!arguments.length) return foldChangeThresholdRight;
        foldChangeThresholdRight = value;
        return chart;
    };

    chart.foldChangeThresholdLeft = function(value) {
        if (!arguments.length) return foldChangeThresholdLeft;
        foldChangeThresholdLeft = value;
        return chart;
    };
